...
import { Utensils, AlertTriangle, ArrowRight, Table as TableIcon, Lock, CheckCircle2, Eye, ShieldCheck, Clock } from 'lucide-react';
...
export default function QrLandingPage() {
    ...
    const resolve = async () => {
      ...
      try {
        ...
        setSessionContext({
          ...
          operatingStatus: res.operatingStatus || null,
        });

        // REPEAT SCAN CHECK: If guest is already the verified Host of this table session, bypass landing choice screen
        ...
        if (isAlreadyHost && !res.operatingStatus?.isClosed) {
  ...
  const isRestaurantClosed = Boolean(session?.operatingStatus?.isClosed);
        ...
        {!isLoading && !error && session && (
          <>
            {isRestaurantClosed ? (
              ...
              /* REGULAR OPEN MODE: Table Status & Ordering Actions */
              <div className="space-y-5 py-2">
