...
import { Plus, Minus, Sparkles, Flame, ChevronRight } from 'lucide-react';
...
const CustomerMenuCard = memo(function CustomerMenuCard({ item }) {
  ...
  return (
    <div
      ...
      className={`bg-card border border-border/80 rounded-2xl p-3.5 flex gap-3.5 hover:border-primary/50 transition-all shadow-xs ${
    ...
    >
      ...
      <div className="relative w-28 h-28 rounded-xl overflow-hidden shrink-0 bg-muted">
        {item.imageCover ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-xs text-primary/70 bg-gradient-to-br from-primary/10 to-accent/10 font-bold p-2 text-center font-display">
            <Sparkles size={20} className="mb-1 text-primary/50" />
            DineSync AI
          </div>
        )}
        ...
        {item.isPopular && (
          <span className="absolute bottom-1.5 left-1.5 bg-amber-500/90 text-white text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5 shadow-xs backdrop-blur-xs">
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        <div>
          <div className="flex items-start justify-between gap-1">
            <h4 className="text-sm sm:text-base font-bold font-display text-foreground leading-snug line-clamp-1">
              {item.name}
            </h4>
          </div>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          {canAdd && (
            cartQuantity > 0 ? (
              <div className="flex items-center border border-primary/40 rounded-xl bg-primary/10 p-0.5 overflow-hidden shadow-xs">
                <button
                  type="button"
                  onClick={handleDecrement}
                  className="w-8 h-8 flex items-center justify-center text-primary font-bold hover:bg-primary/20 transition-colors rounded-lg active:scale-95 touch-manipulation"
                  aria-label="Decrease quantity"
                >
                  <Minus size={14} />
                </button>
                <span className="px-2 min-w-[20px] text-center text-xs font-bold font-mono text-foreground">
                  {cartQuantity}
                </span>
                <button
                  type="button"
              ...
              <Button
                ...
                className="h-9 min-w-[80px] text-xs font-bold gap-1 px-3.5 rounded-xl shadow-xs"
              >
                <Plus size={14} /> Add
              </Button>
            )
